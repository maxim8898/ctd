...
const DatePicker: React.FC<DatepickerProps> = ({timestamp, changeDate}) => {
  // Transform timestamp to 'YYYY-mm-dd' format.
  let defaultDate: Date = new Date(timestamp);
  defaultDate.setMinutes(defaultDate.getMinutes() - defaultDate.getTimezoneOffset());
  const date: string = defaultDate.toJSON().slice(0, 10);

  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    changeDate(new Date(e.target.value).getTime());
  }

  return (
    <div className="relative my-3 mx-auto w-72">
      <input id="id-date07" type="date" name="id-date07"
